# Grammar 3 - V-(으)ㄹ래요

Unidad: [[Korean 2A/2B/Unidad 10/Index - Unidad 10|Unidad 10: 뭐 먹을래?]]
Fuente: Student Book 2B + audios de la app koreano.

## Idea central
`V-(으)ㄹ래요` expresa voluntad, eleccion o pregunta de deseo: quiero/voy a... o ¿quieres...?

No es futuro neutro. Tiene sabor de decision personal, eleccion inmediata o invitacion.

## La idea clave
La idea clave: `-(으)ㄹ래요` no es futuro neutro; es deseo/decision del hablante o pregunta por lo que el oyente quiere hacer ahora.

## Forma
| Tipo | Forma | Ejemplo |
|---|---|---|
| Verbo sin 받침 | `V-ㄹ래요` | 마시다 -> 마실래요 |
| Verbo con 받침 | `V-을래요` | 먹다 -> 먹을래요 |
| Informal | `-(으)ㄹ래` | 뭐 먹을래? |

## Como pensarlo
La persona cambia el sentido:

- Primera persona: `저는 비빔밥을 먹을래요.` = Yo quiero/voy a comer bibimbap.
- Segunda persona en pregunta: `뭐 먹을래요?` = ¿Que quieres comer?
- Invitacion: `한번 먹어 볼래요?` = ¿Quieres probarlo?

Para presentaciones o situaciones formales, mejor `-겠습니다`: `소개하겠습니다`, no `소개할래요`.

## Ejemplos base y variantes
| Coreano | Espanol |
|---|---|
| 뭐 먹을래요? | ¿Que quieres comer? |
| 저는 비빔밥을 먹을래요. | Yo quiero/voy a comer bibimbap. |
| 커피 마실래? | ¿Quieres tomar cafe? |
| 이 식당에서 한번 먹어 볼래요? | ¿Quieres probar comer en este restaurante? |

## Ejemplos de la unidad con links
| Coreano | Espanol | Audio / nota |
|---|---|---|
| 뭐 먹을래? | ¿Que vas a comer? | [[Korean 2A/2B/Unidad 10/Grammar Audio/Track 05 - 문법|Track 05 · 문법]] |
| 나는 칼국수 먹을래. 너는? | Yo voy a comer kalguksu. ¿Y tu? | [[Korean 2A/2B/Unidad 10/Grammar Audio/Track 05 - 문법|Track 05 · 문법]] |
| 스티븐, 뭐 먹을래? | Steven, ¿que vas a comer? | [[Korean 2A/2B/Unidad 10/Speaking/Speaking 2 - 말하기|Track 07 · 말하기]] |
| 이 식당은 삼계탕이 맛있는데 한번 먹어볼래? | Este restaurante es bueno para samgyetang, ¿quieres probarlo? | [[Korean 2A/2B/Unidad 10/Speaking/Speaking 2 - 말하기|Track 07 · 말하기]] |

## Audios conectados
- [[Korean 2A/2B/Unidad 10/Grammar Audio/Track 05 - 문법|Track 05 · 문법]]
- [[Korean 2A/2B/Unidad 10/Speaking/Speaking 2 - 말하기|Track 07 · 말하기]]
- [[Korean 2A/2B/Unidad 10/Listening/Listening 1 - 듣기|Track 08 · 듣기]]
