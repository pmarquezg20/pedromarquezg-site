---
title: pedromarquezg
---

<div class="hub">
  <p class="hub-kicker">PORTAFOLIO · 안녕하세요 👋</p>
  <p class="hub-sub">Mi espacio de coreano — y el comienzo de mi portafolio. Elige por dónde entrar.</p>
</div>

<a class="hub-featured" href="/Korean-3/Index">
  <span class="hub-featured-tag">EN CURSO · 3급</span>
  <span class="hub-featured-title">📗 Nivel 3 — 서울대 한국어 3A</span>
  <span class="hub-featured-desc">Unidades 1–6 desglosadas: 23 reglas de gramática con el appendix del libro y los errores típicos, 425 palabras con traducción, los transcripts de todos los audios y las seis lecturas.</span>
  <span class="hub-featured-meta">1과 인용문 · 2과 습관 · 3과 날씨 · 4과 음식 · 5과 쇼핑 · 6과 약속</span>
  <span class="hub-card-go">Abrir el índice →</span>
</a>

<div class="hub-featured-links">
  <a href="/Korean-3/3A/Unidad-1/Index---Unidad-1">1과 · 인용문</a>
  <a href="/Korean-3/3A/Unidad-2/Index---Unidad-2">2과 · 습관</a>
  <a href="/Korean-3/3A/Unidad-3/Index---Unidad-3">3과 · 날씨</a>
  <a href="/Korean-3/3A/Unidad-6/Index---Unidad-6">6과 · 약속</a>
  <a href="https://koreano.pedromarquezg.com/">🎧 Estudiar en la app</a>
</div>

<div class="hub-grid">
  <a class="hub-card" href="/Korean-3/Index">
    <span class="hub-emoji">📗</span>
    <span class="hub-card-title">Nivel 3</span>
    <span class="hub-card-desc">서울대 한국어 3A y 3B: gramática, vocabulario, transcripts y lecturas.</span>
    <span class="hub-card-go">Entrar →</span>
  </a>
  <a class="hub-card" href="/Korean-2A/Index">
    <span class="hub-emoji">📓</span>
    <span class="hub-card-title">Nivel 2</span>
    <span class="hub-card-desc">Gramática, vocabulario y machetes de SNU Korean 2A y 2B.</span>
    <span class="hub-card-go">Entrar →</span>
  </a>
  <a class="hub-card" href="https://koreano.pedromarquezg.com/">
    <span class="hub-emoji">🎧</span>
    <span class="hub-card-title">La app</span>
    <span class="hub-card-desc">Audio con el texto marcado, repaso espaciado de vocabulario, lectura cronometrada y mini parcial por unidad.</span>
    <span class="hub-card-go">Abrir app →</span>
  </a>
  <a class="hub-card" href="/flashcards/" data-router-ignore>
    <span class="hub-emoji">🃏</span>
    <span class="hub-card-title">Flashcards</span>
    <span class="hub-card-desc">Las 35 reglas de gramática de 2B con método Leitner: repite las que fallas, descarta las que ya sabes.</span>
    <span class="hub-card-go">Jugar →</span>
  </a>
</div>

---

##### Atajos rápidos
- [[Korean 3/Index|📗 Nivel 3]] — 3A y 3B, unidad por unidad
- <a href="https://koreano.pedromarquezg.com/">🎧 La app</a> — audio, 단어, 읽기 y 시험 de cada unidad
- <a href="/flashcards/" data-router-ignore>🃏 Flashcards de gramática 2B</a> — las 35 reglas, modo Leitner
- [[Korean 2A/2B/Examen Final 2B/Index - Examen Final 2B|🎯 Examen Final 2B]] — checklist, 쓰기 y 말하기
- [[Korean 2A/Index|📚 Vault de coreano]] — índice general (2A y 2B)
- [[Korean 2A/2B/Index|📔 Unidades 10–18]] — 2B unidad por unidad
- [[Korean 2A/2B/문법 총정리 - 2B Grammar Reference|📘 Grammar Reference 2B]] — gramática U10–18
- [[Korean 2A/2A/Cheat Sheet - 문법 U1-U9|📗 Cheat Sheet 2A]] — gramática U1–9

> [!info] Sobre este sitio
> Notas personales de estudio. El material de referencia procede del libro *Seoul National University Korean*. Ver [[Aviso|aviso y atribución]].
